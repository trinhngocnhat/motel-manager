...
@Composable
fun TenantsManagementScreen(navController: NavController) {
    Scaffold(
        topBar = { CommonHeader(roleName = "QUẢN LÝ KHÁCH THUÊ", navController = navController, showBackButton = true) }
    ) { innerPadding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Text(text = "Trang quản lý khách thuê")
            MotelTenantCardList()
            // TODO: Add Tenants Management UI components here
        }
    }
}

@Composable
fun MotelTenantCardList()
{
